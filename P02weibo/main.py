...
def raise_for_banned(head, url):
    if head.get("SERVER", None) == 'nginx' or "need_login" in url:
...
if __name__ == "__main__":
    ...
    query_content = "泛式"
    with XlsWriter(f"{query_content}_微博数据.xls", name) as wr:
        for i in tqdm(range(1, 51)):
            ...
            for feed in feed_list_items:
                ...
                if like_count == '赞':
                    like_count = 0
                wr.add_item([blog_id, blogger_name, blogger_url, verified_type, is_weibo_vip, blog_content, post_time,
